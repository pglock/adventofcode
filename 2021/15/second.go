...
func readData(filename string) (map[position]int, int, int) {
	...
	nCols = nCols + 1
	expanded := make(map[position]int)
	for i:=0; i<5; i++ {
		for j:=0; j<5; j++ {
			for pos, risk := range array {
				newPos := position{pos.row + nRows * i, pos.col + nCols * j}
				expanded[newPos] = (risk + i + j) - 9 * ((risk+i+j) / 10)
			}
		}
	}
	return expanded, nRows * 5, nCols * 5
}
